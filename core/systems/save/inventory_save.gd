...
func on_save_game(saved_data_array: Array[SavedData]) -> void:
	if Global.hot_bar:
		...
		hotbar_entry.payload = CardSaveUtil.serialize_grid(Global.hot_bar)
		saved_data_array.append(hotbar_entry)
	if Global.player_inv:
		var inv_entry := SavedData.new()
		inv_entry.identifier = ID_PLAYER_INV
		inv_entry.payload = CardSaveUtil.serialize_grid(Global.player_inv)
		saved_data_array.append(inv_entry)


func on_load_game(saved_data_array: Array[SavedData]) -> void:
	for data in saved_data_array:
		if data.identifier == ID_HOTBAR and Global.hot_bar:
			CardSaveUtil.apply_grid_payload(Global.hot_bar, data.payload)
		elif data.identifier == ID_PLAYER_INV and Global.player_inv:
			CardSaveUtil.apply_grid_payload(Global.player_inv, data.payload)
